function fminsearch(fun,Parm0,x,y,Opt){// fun = function(x,Parm)
	...
	if(!Opt.objFun){Opt.objFun=function(y,yp){console.log('new sum');return y.map(function(yi,i){let a = Math.pow((yi-yp[i]),2);console.log(a);return a}).reduce(function(a,b){return a+b})}} // SSD default objective function being minimized
	let regModel={}
...
function fminsearch2(fun,Parm0,points,Opt){// fun = function(x,Parm)
	...
	if(typeof(Opt.display)=='undefined'){Opt.display='console'};
    if(typeof(Opt.loglevel)=='undefined'){Opt.loglevel='normal'};
	...
	if(!Opt.objFun){Opt.objFun=function(result){debug('new sum'); return result.map((z)=> {debug(z); return Math.abs(z[2])}).reduce(function(a,b){return a+b})}};
    let regModel={}
	var ya,y0,yb,fP0,fP1;
	var P0=[...Parm0],P1=[...Parm0]; // clone parameter array to decouple passing by reference
	var n = P0.length;
	var step=Opt.step;
	function funEval(P){return Opt.objFun(fun(points,P))}//function evaluation for curent Parameter values to determine value of objective function, passed as a Opt parameter (Opt.objFun)
	// silly multi-univariate walk
	// assemble regresssion Model
	regModel={
		Opt:Opt,
...
let x = [1,2,3,4];
let y = [1,2,2,1];
combine = function(x,y) {z=[]; for (let i = 0; i < x.length; i++) {z[i]=[x[i],y[i]]}; return z}
...
let fun = function(x,P){return x.map(function(xi){return (P[1]+Math.sqrt(Math.pow(P[2],2)-Math.pow((xi-P[0]),2)))})};
let fun2 = function(points,P){return points.map(function(p){return [p[0],p[1],((p[0]-P[0])**2+(p[1]-P[1])**2-P[2]**2)]})};
...
y = [2,2,2,2,2,2,2,2];
hline = combine(x,y);
let Parms4=fminsearch2(fun2,[50,50,49],hline, {maxIter:100,loglevel:"norm"});
//console.log(Parms);
console.log(Parms4);
//
